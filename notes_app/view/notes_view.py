...
APP_TITLE = "QuillNotes"
APP_METADATA_ROWS = [
    "A simple notes application",
    "built with Python 3.8 & KivyMD,and coverted to a desktop app with PyInstaller, and also convert to a mobile app with by group 3, Software Construction",
...
# this is a shareable button that allows the user to share the note

    def press_menu_item_share_note(self):
        current_note_content = self.text_section_view.text
        note_file_path = "note.txt"

        # Save the current note to a temporary file
        with open(note_file_path, "w") as note_file:
            note_file.write(current_note_content)

        # Upload the file to Dropbox and get a shareable link
        shareable_link = self.upload_to_dropbox(note_file_path)
        if shareable_link:
            pyperclip.copy(shareable_link)
            self.show_snackbar("Shareable link copied to clipboard!")
        else:
            self.show_error_bar("Failed to generate shareable link.")

    def upload_to_dropbox(self, file_path):
        dbx = dropbox.Dropbox(self.settings.DROPBOX_API_TOKEN)
        with open(file_path, "rb") as f:
            try:
                dbx.files_upload(f.read(), f"/{file_path}", mute=True)
    ...
    def press_menu_storage_item_callback(self, text_item):
        ...
        elif text_item == MenuStorageItems.ShareNote.value:  # Handle the new menu item
            self.press_menu_item_share_note()
